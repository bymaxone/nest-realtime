...
- copilot-followup ✅ 2026-06-27 — Resolved post-merge Copilot review on PR #8: corrected the inverted cacheTtlMs warning wording (an over-long cache outlives the interval so revalidate is skipped and reauth is weakened, not "miss every cycle"); forRootAsync now rejects zero/multiple async-options patterns with an actionable INVALID_OPTIONS error instead of injecting undefined; bearer fixture spec uses transport 'websocket'. 100% coverage retained; all gates green.
